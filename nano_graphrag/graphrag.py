    ...
    async def ainsert(self, string_or_strings):
        ...
        try:
            ...
            if not len(inserting_chunks):
                logger.warning(f"All chunks are already in the storage")  # 所有块都已存在于存储中的警告
                return  # 如果没有新块，直接返回
            logger.info(f"[New Chunks] inserting {len(inserting_chunks)} chunks")  # 记录插入的新块数量
            
            # 如果启用了简单RAG模式，将文本块插入到向量数据库中
            if self.enable_naive_rag:
                logger.info("Insert chunks for naive RAG")  # 记录为简单RAG插入块的日志
            ...
            # ---------- 提取/总结实体并插入到图中
            logger.info("[Entity Extraction]...")  # 记录开始实体提取的日志
            ...
            if maybe_new_kg is None:
                logger.warning("No new entities found")  # 没有找到新实体的警告
                return  # 如果没有新实体，直接返回
            self.chunk_entity_relation_graph = maybe_new_kg  # 更新块-实体关系图
            
            # ---------- 更新图的聚类
            logger.info("[Community Report]...")  # 记录开始社区报告生成的日志
            # 对知识图谱进行聚类分析，识别实体社区
